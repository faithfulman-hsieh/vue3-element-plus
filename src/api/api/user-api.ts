...
export const UserAPIAxiosParamCreator = function (configuration?: Configuration) {
    return {
        ...
        // ★★★ [FCM Token Storage] Manual Patch: updateFcmToken ★★★
        /**
         * Updates FCM Token for the current user
         * @summary Update FCM Token
         * @param {string} token The FCM token string
         * @param {*} [options] Override http request option.
         * @throws {RequiredError}
         */
        updateFcmToken: async (token: string, options: RawAxiosRequestConfig = {}): Promise<RequestArgs> => {
            const localVarPath = `/api/fcm-token`;
            const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
            let baseOptions;
            if (configuration) {
                baseOptions = configuration.baseOptions;
            ...
            // Backend expects Map<String, String> like { "token": "..." }
...
export const UserAPIFp = function(configuration?: Configuration) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        async updateFcmToken(token: string, options?: RawAxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<string>> {
            const localVarAxiosArgs = await localVarAxiosParamCreator.updateFcmToken(token, options);
            // Reusing existing server map logic (fallback to default)
            const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
            const localVarOperationServerBasePath = operationServerMap['UserAPI.updateFcmToken']?.[localVarOperationServerIndex]?.url;
            return (axios, basePath) => createRequestFunction(localVarAxiosArgs, globalAxios, BASE_PATH, configuration)(axios, localVarOperationServerBasePath || basePath);
        },
    }
};
...
export const UserAPIFactory = function (configuration?: Configuration, basePath?: string, axios?: AxiosInstance) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        updateFcmToken(token: string, options?: RawAxiosRequestConfig): AxiosPromise<string> {
            return localVarFp.updateFcmToken(token, options).then((request) => request(axios, basePath));
        },
    };
};
...
export class UserAPI extends BaseAPI {
     ...
     * @memberof UserAPI
     */
    public updateFcmToken(token: string, options?: RawAxiosRequestConfig) {
        return UserAPIFp(this.configuration).updateFcmToken(token, options).then((request) => request(this.axios, this.basePath));
    }
}